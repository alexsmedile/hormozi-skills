# ENCABEZADOS-CANONICOS.md · hormozi-skills-es

**Este es el documento más crítico del proyecto.** Las plantillas de artefacto están **duplicadas entre buckets con dueños distintos**. Si dos traductores rinden el mismo encabezado de dos formas, los documentos que el sistema genera salen incoherentes y `sub-sales` no encuentra las secciones que `sub-value` escribió.

## Duplicaciones detectadas (verificadas en el fuente)
...
1. **La columna ES es obligatoria y literal.** No se "adapta" ni se mejora.
2. **Se conserva el nivel de encabezado** (`#`, `##`, `###`, `####`) y **la numeración** (`## 7.`) exactamente como en el fuente. La tabla lista solo el texto.
3. **Mayúscula inicial solamente** (sentence case), aunque el inglés use Title Case. Excepciones: nombres propios (`Hormozi`), siglas (`CTA`, `DFY`, `FAQ`, `DM`), nombres de artefacto (`OFFER.md`) y el nombre de framework `Ecuación de Valor`.
4. **Los nombres de artefacto `*.md` nunca se traducen** (ver `NO-TRADUCIR.txt`).
5. Los **placeholders entre corchetes** (`[copy]`, `[name]`, `[what they see]`) se traducen por dentro pero conservan los corchetes: `[copy]` → `[copy]` (es término KEEP), `[name]` → `[nombre]`.
6. Se conservan los **glifos de comilla del fuente**: `"How I" Hooks` (comillas rectas, `sub-sales`) → `Hooks de "Cómo yo"`; `“How I” Hooks` (comillas tipográficas, `hormozi-hooks`) → `Hooks de “Cómo yo”`.

**Recuento medido:**

| Sección | Rótulos | En ≥2 archivos |
|---|---|---|
| A · Encabezados de plantilla (dentro de ` ```md ` + bloques de plantilla sin fence de `hormozi-hooks` e `idea-to-product`) | **278** | **87** |
| B · Encabezados de estructura de `SKILL.md` / `agents/*.md` | 48 | 13 |
...
Total de encabezados `^#{1,4}` distintos en el corpus (dentro y fuera de fences, sin normalizar la numeración): **683**. Normalizados (quitando el prefijo `N.`): **629**.

---

## A. Encabezados de plantilla (dentro de ` ```md ` → van a los artefactos generados)

Ordenados por número de archivos, descendente.

| Encabezado EN | Encabezado ES | Archivos donde aparece |
|---|---|---|
...
## B. Encabezados de estructura del propio `SKILL.md` / `agents/*.md` (fuera de fences)

Estos **no** van al artefacto generado, pero son la arquitectura visible de los 26 archivos. Una traducción única, obligatoria.
...
## C. Pseudoencabezados en texto plano (después de los separadores `⸻`)

En 19 archivos, la mitad inferior del documento abandona los `#` y usa **líneas sueltas en texto plano** como títulos. Son encabezados de facto y **deben traducirse igual en todos lados**.
...
## D. Etiquetas de viñeta con dos puntos (dentro de plantillas)

Traducción única obligatoria. Todas van con **mayúscula inicial y dos puntos**, igual que el fuente. Los `- Etiqueta:` sin valor mantienen los dos puntos aunque no haya texto detrás.

| Etiqueta EN | Etiqueta ES | Archivos donde aparece |
|---|---|---|
...
## E. Cabeceras de columna de tabla

| Cabecera EN | Cabecera ES | Archivos |
|---|---|---|
...
## F. Bloques literales del orquestador (plantillas de salida a consola)

Estas cadenas se emiten tal cual al usuario. Traducción fija, **una sola vez**.

| EN | ES |
|---|---|
| `DETECTED STAGE:` | `ETAPA DETECTADA:` |
| `SKILLS THAT WILL RUN:` | `SKILLS QUE SE EJECUTARÁN:` |
| `ESTIMATED OUTPUT:` | `SALIDA ESTIMADA:` |
| `Confirm to proceed, or tell me what to change.` | `Confirma para continuar, o dime qué cambiar.` |
| `ORCHESTRATOR BRIEF:` | `BRIEF DEL ORQUESTADOR:` |
| `USER CONTEXT:` | `CONTEXTO DEL USUARIO:` |
| `EXISTING FILES:` | `ARCHIVOS EXISTENTES:` |
| `YOUR TASK:` | `TU TAREA:` |
| `BRIEF:` | `BRIEF:` |
| `- Business/idea:` | `- Negocio/idea:` |
...
| `- Existing proof:` | `- Pruebas existentes:` |
| `- Existing assets:` | `- Recursos existentes:` |
| `- Constraints:` | `- Restricciones:` |
| `- Stage:` | `- Etapa:` |
| `- Session goal:` | `- Meta de la sesión:` |
| `- Idea/Business:` | `- Idea/Negocio:` |
| `- Raw audience guess:` | `- Audiencia supuesta (en bruto):` |
| `- Skills/expertise:` | `- Habilidades/experiencia:` |
| `- Known pain points:` | `- Puntos de dolor conocidos:` |
| `- Known objections:` | `- Objeciones conocidas:` |
| `- Offer:` | `- Oferta:` |
| `- Value stack total:` | `- Total del stack de valor:` |
| `- Target audience purchasing power:` | `- Poder adquisitivo de la audiencia objetivo:` |
| `- User constraints:` | `- Restricciones del usuario:` |
| `- Write to output/ folder` | `- Escribe en la carpeta output/` |
| `- Report back with 3 key findings` | `- Reporta 3 hallazgos clave` |
| `[STRONG / MODERATE / WEAK]` | `[FUERTE / MODERADA / DÉBIL]` |
| `[DIY / DWY / DFY / Hybrid]` | `[DIY / DWY / DFY / Híbrido]` |
| `[A / B / C / D / E]` | `[A / B / C / D / E]` |
| `[idea only / rough offer / existing product]` | `[solo idea / oferta en borrador / producto existente]` |
| `[template / checklist / call / etc.]` | `[plantilla / checklist / llamada / etc.]` |
| `[low / mid / high]` | `[bajo / medio / alto]` |
| `[time, money, energy]` | `[tiempo, dinero, energía]` |
| `[time, budget, energy]` | `[tiempo, presupuesto, energía]` |
